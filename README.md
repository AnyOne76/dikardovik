...
**Docker:** при старте контейнера после миграций выполняется `npm run db:ensure-admin` — если в базе ещё нет пользователя с ролью `admin`, создаётся администратор из тех же переменных `SEED_ADMIN_*`. Если админ уже есть, пароль не перезаписывается. Вручную принудительно обновить пароль админа из `.env`: `npm run db:seed`.

**Не входит:** проверьте, что в браузере открываете тот же хост и порт, что в `NEXTAUTH_URL` (для LAN — `http://10.x.x.x:25531`, а не только `localhost`). Логин — значение `SEED_ADMIN_LOGIN` (по умолчанию `hr-admin`), поле на странице может называться «Email», но вводится именно логин.
