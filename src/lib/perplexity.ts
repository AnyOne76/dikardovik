  ...
  const perplexityLines = await queryPerplexity(
    ...
    (() => {
      const base = [
        ...
        "Убирай общие фразы без смысла (например: 'выполняет поручения руководства') — только если нет иных вариантов.",
      ];

      const s = (opts.sectionHuman ?? "").toLowerCase();
      const isDuties =
        s.includes("работник обязан") ||
        s.includes("должностн") ||
        s.includes("обязанност") ||
        s.includes("duties");
